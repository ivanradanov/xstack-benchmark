...
__global__ void kernel_A_mul_B(int ni, int nj, int nk,
                               double *C,
                               double *A,
                               double *B) {
...
int main(int argc, char** argv)
{
  ...
  hipMalloc(&dev_C, nl*nj*sizeof(double));
  hipMalloc(&dev_D, ni*nl*sizeof(double));
  hipMalloc(&dev_E, ni*nj*sizeof(double));
  hipMalloc(&dev_F, nj*nl*sizeof(double));
  hipMalloc(&dev_G, ni*nl*sizeof(double));
  hipMemcpy(dev_A, A, ni*nk*sizeof(double), hipMemcpyHostToDevice);
  hipMemcpy(dev_B, B, nk*nj*sizeof(double), hipMemcpyHostToDevice);
  hipMemcpy(dev_C, C, nl*nj*sizeof(double), hipMemcpyHostToDevice);
  hipMemcpy(dev_D, D, ni*nl*sizeof(double), hipMemcpyHostToDevice);
  hipMemcpy(dev_E, E, ni*nj*sizeof(double), hipMemcpyHostToDevice);
  hipMemcpy(dev_F, F, nj*nl*sizeof(double), hipMemcpyHostToDevice);
  hipMemcpy(dev_G, G, ni*nl*sizeof(double), hipMemcpyHostToDevice);


  kernel(ni, nj, nk, nl, nm, dev_E, dev_A, dev_B, dev_F, dev_C, dev_D, dev_G);


  hipMemcpy(G, dev_G, ni * nl * sizeof(double), hipMemcpyDeviceToHost);
