...
static void
kernel(int n, double *A) {
  const unsigned int threadsPerBlock = 256;

  for (int iter = 0; iter < n - 1; iter++) {
    kernel_div<<<num_blocks(n - (iter + 1), threadsPerBlock), threadsPerBlock>>>(n, A, iter);


    dim3 block(threadsPerBlock / 32, 32, 1);
    dim3 grid(num_blocks(n - (iter + 1), block.x), num_blocks(n - (iter + 1), block.y), 1);
