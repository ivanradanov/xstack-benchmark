...
int main(int argc, char** argv)
{
  ...
  double *dev_oldtable;
  double *dev_seq;
  hipMalloc(&dev_table, n*n*sizeof(double));
  hipMalloc(&dev_oldtable, n*n*sizeof(double));
  hipMalloc(&dev_seq, n*sizeof(double));
  hipMemcpy(dev_table, table, n*n*sizeof(double), hipMemcpyHostToDevice);
  hipMemcpy(dev_oldtable, oldtable, n*n*sizeof(double), hipMemcpyHostToDevice);
  hipMemcpy(dev_seq, seq, n*sizeof(double), hipMemcpyHostToDevice);


  kernel(n, dev_seq, dev_table, dev_oldtable);
  hipMemcpy(table, dev_table, n*n*sizeof(double), hipMemcpyDeviceToHost);
