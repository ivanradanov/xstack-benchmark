...
static void kernel(int tsteps, int iter, double *A, double *B) {
  ...
  for (int t = 1; t <= tsteps; t++) {
    ...
    dim3 grid(num_blocks(iter - 2, block.x), num_blocks(iter - 2, block.y), num_blocks(iter - 2, block.z));
...
int main(int argc, char** argv)
{
  ...
  double *dev_A;
  double *dev_B;
  hipMalloc(&dev_A, n*n*n*sizeof(double));
  hipMalloc(&dev_B, n*n*n*sizeof(double));
  hipMemcpy(dev_A, A, n*n*n*sizeof(double), hipMemcpyHostToDevice);
  hipMemcpy(dev_B, B, n*n*n*sizeof(double), hipMemcpyHostToDevice);


  kernel(tsteps, n, dev_A, dev_B);
  hipMemcpy(A, dev_A, n*n*n*sizeof(double), hipMemcpyDeviceToHost);
