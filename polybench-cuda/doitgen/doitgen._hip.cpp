...
__global__ void kernel_sum(int nr, int nq, int np,
                           double *A,
...
int main(int argc, char** argv)
{
  ...
  double *dev_sum;
  double *dev_C4;
  hipMalloc(&dev_A, nr*nq*np*sizeof(double));
  hipMalloc(&dev_sum, nr*nq*np*sizeof(double));
  hipMalloc(&dev_C4, np*np*sizeof(double));
  hipMemcpy(dev_A, A, nr*nq*np*sizeof(double), hipMemcpyHostToDevice);
  hipMemcpy(dev_C4, C4, np*np*sizeof(double), hipMemcpyHostToDevice);
  hipMemcpy(dev_sum, sum, nr*nq*np*sizeof(double), hipMemcpyHostToDevice);
  ...
  hipMemcpy(sum, dev_sum, nr*nq*np*sizeof(double), hipMemcpyDeviceToHost);
